...
def process_image(imgTrain, imgToProcess, output_path, original_size):
    ...
    encodeimg_list = face_recognition.face_encodings(imgTrain)
    if len(encodeimg_list) == 0:
        raise ValueError("No faces found in the training image.")
    encodeimg = encodeimg_list[0]
    ...
    for face_encoding, face_location in zip(face_encodings, face_locations):
        match = face_recognition.compare_faces([encodeimg], face_encoding, tolerance=0.4)
